...
void Cpp_sampler(   const arma::mat& Y_data, const arma::mat& X_data, int maxiter,
  ...
  for( iter=0; iter<maxiter; iter++){
    ...
    file_name = "./samples/xpi/" + std::to_string(iter+1) + ".data";
    xpi_curr.save( file_name, arma::raw_ascii );
    file_name = "./samples/nodepi/" + std::to_string(iter+1) + ".data";
    nodepi_curr.save( file_name, arma::raw_ascii );
    file_name = "./samples/t/" + std::to_string(iter+1) + ".data";
    t_curr.save( file_name, arma::raw_ascii );
